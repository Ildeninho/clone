...
const produtos = [
    {
        nome: "Regata Branca Masculina",
        imagem: "./assets/regatabranca.jpg",
        precoOriginal: 99.90,
        precoPix: 49.90,
        precoCartao: 59.90,
        desconto: "-42%",
    ...
    {
        nome: "Regata Branca Masculina",
        imagem: "./assets/regatamalhacinza.jpg",
        precoOriginal: 99.90,
        precoPix: 49.90,
        precoCartao: 59.90,
        desconto: "-42%",
...
function filtrarProdutos() {
    ...
    filtrados.forEach(produto => {
        container.innerHTML += `
        <div class="product">
          <div class="text-center product bg-white p-4 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 relative cursor-pointer">
            <img src="${produto.imagem}" alt="${produto.nome}" class="w-full object-contain h-48 mb-4 rounded-md">
            <div class="bg-black text-white text-sm font-bold px-6 py-1 rounded-md w-max mb-2 mx-auto">${produto.desconto} OFF</div>
            <p class="font-semibold text-lg leading-tight mb-1">${produto.nome}</p>
            <div class="flex justify-center items-center text-yellow-500 text-sm gap-1">
              <span>☆☆☆☆☆</span>
              <span class="text-gray-600 text-xs">(${produto.avaliacao})</span>
            </div>
            <p class="text-sm text-gray-400 line-through">R$ ${produto.precoOriginal.toFixed(2)}</p>
            <p class="text-xl font-bold text-red mb-1">R$ ${produto.precoPix.toFixed(2)} <span class="text-sm font-normal">no Pix</span></p>
            <p class="text-sm text-gray-600">ou 1x de R$ ${produto.precoCartao.toFixed(2)}</p>
          </div>
        </div>
      `;
    });
...
document.querySelectorAll('.size-option').forEach(button => {
    button.addEventListener('click', function () {
        ...
        this.classList.add('bg-emerald-600', 'border-emerald-700');
